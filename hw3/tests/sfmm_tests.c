...
Test(sfmm_student_suite, realloc_same, .timeout = TEST_TIMEOUT) {
	void *x = sf_malloc(sizeof(double));
	void *y = sf_realloc(x, sizeof(double));

	cr_assert_not_null(y, "y is null!!");
	cr_assert(x == y, "payload addresses are different!!");

	sf_block *block = (sf_block*) ((char*) y - 2*sizeof(sf_header));
	cr_assert((block->header^MAGIC) & THIS_BLOCK_ALLOCATED, "allocated bit not set!!");
	cr_assert((block->header^MAGIC) == (32+4), "unexpected header bro");
...
Test(sfmm_student_suite, free_small_block_into_full_quick_list, .timeout = TEST_TIMEOUT) {
	sf_errno = 0;
	void* b1 = sf_malloc(sizeof(double));
	void* b2 = sf_malloc(sizeof(double));
	void* b3 = sf_malloc(sizeof(double));
	void* b4 = sf_malloc(sizeof(double));
	void* b5 = sf_malloc(sizeof(double));
	void* b6 = sf_malloc(sizeof(double));
	...
	assert_quick_list_block_count(0, 1); //there shoulf be one block in the quick list
	assert_quick_list_block_count(32, 1); //of size 32
	assert_free_block_count(160, 1); //with all those blocks coalesced
	assert_free_block_count(3888, 1); //the big block

	cr_assert(sf_errno == 0, "sf_errno is not zero!");
}

Test(sfmm_student_suite, one_block_no_free_blocks, .timeout = TEST_TIMEOUT) {
	sf_errno = 0;
	void *x = sf_malloc(4080-16);

	cr_assert_not_null(x, "why is x null bro");
...
Test(sfmm_student_suite, free_into_two_quick_lists, .timeout = TEST_TIMEOUT) {
	sf_errno = 0;
	...
	assert_quick_list_block_count(32, 1); // one block in quick list of size 32
	assert_quick_list_block_count(48, 1); // one block in quick list of size 48s
	assert_free_block_count(0, 1);
	assert_free_block_count(4000, 1); //there should be one free block in free list
}
